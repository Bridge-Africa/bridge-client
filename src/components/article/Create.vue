<template>
    <el-form ref="form" :model="form" label-width="120px" :rules="rules">
      ...
      <el-form-item>
        <el-button :loading="loading" type="primary" @click="create">{{ $t('article.button.create') }}</el-button>
      </el-form-item>
    </el-form>
</template>
  ...
  methods: {
    ...
    create () {
      this.$refs.form.validate((valid) => {
        if (valid) {
          this.loading = true
          this.$articleService.createArticle(Object.assign({}, this.form))
            .then(data => {
              this.displayMessage(this.$t('article.message.created'))
              this.go('articles')
            })
